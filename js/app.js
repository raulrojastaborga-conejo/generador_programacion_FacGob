...
const files = {
  oferta: 'fileOferta', disponibilidad: 'fileDisponibilidad', salas: 'fileSalas', bloques: 'fileBloques', demanda: 'fileDemanda', restricciones: 'fileRestricciones'
};

function setStatus(html){ document.getElementById('inputStatus').innerHTML = html; }
...
async function loadInputs(){
  ...
  const html = validations.map(v => v.ok
    ...
    : `<p><span class="badge dura">Revisar</span> ${v.name}: faltan columnas ${v.missing.join(', ') || '—'}.</p>`).join('');
  setStatus(html);
}
...
+/).map(x=>x.trim()).filter(Boolean);
  lines.forEach((line,i)=>rules.push({id:`local-${i+1}`, fuente:'local', regla_original:line, tipo:'criterio_general', dureza: line.toLowerCase().includes('debe') || line.toLowerCase().includes('no ') ? 'dura' : 'blanda'}));
  return rules;
}
...
function interpretRulesWithJsonp(url, text){
  return new Promise((resolve,reject)=>{
    ...
    window[cb] = (payload) => { delete window[cb]; script.remove(); resolve(payload); };
    const script = document.createElement('script');
    script.onerror = () => { delete window[cb]; script.remove(); reject(new Error('No fue posible llamar al backend Apps Script.')); };
