function expandOferta(oferta, demanda){
  ...
  oferta.forEach(row => {
    ...
    for(let i=1;i<=Math.max(1,n);i++){
      sections.push({...row, seccion:i, cupo: Number(row.cupo_estimado || dem.estimacion_2026 || 0)});
    }
  });
  return sections;
}

function isYes(value){return ['si','sí','s','yes','true','1'].includes(String(value||'').trim().toLowerCase())}
function isNo(value){return ['no','false','0'].includes(String(value||'').trim().toLowerCase())}
...
function generateSchedule(data, structuredRules=[]){
  ...
  sections.sort((a,b) => String(a.prioridad||'').localeCompare(String(b.prioridad||'')));
  ...
  for(const section of sections){
    ...
    if(best){
      ...
      schedule.push({
        carrera: section.carrera, codigo: section.codigo, ramo: section.ramo, seccion: section.seccion,
        profesor, dia: best.bloque.dia, bloque: best.bloque.bloque, inicio: best.bloque.inicio, termino: best.bloque.termino,
        sala: best.sala.sala, cupo: section.cupo, estado: best.penalty ? 'Asignado con advertencia' : 'Asignado'
      });
      if(best.penalty){
        alerts.push({tipo:'Blanda', codigo:section.codigo, ramo:section.ramo, detalle:'Asignación posible, pero no cumple totalmente una regla blanda interpretada.', accion:'Revisar si se acepta la excepción.'});
      }
    }else{
      schedule.push({carrera: section.carrera, codigo: section.codigo, ramo: section.ramo, seccion: section.seccion, profesor: section.profesor_sugerido, estado:'No asignado'});
      alerts.push({tipo:'Dura', codigo:section.codigo, ramo:section.ramo, detalle:'No se encontró combinación profesor/bloque/sala que cumpla restricciones duras mínimas.', accion:'Revisar disponibilidad docente, sala, capacidad o requiere_pc.'});
    }
  }
